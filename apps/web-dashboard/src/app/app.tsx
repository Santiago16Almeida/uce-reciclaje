...
// ✅ Única definición de la IP del Gateway para todo el Dashboard
const API_BASE = 'http://100.50.26.228:3000/api';
      ...
      <main className="flex-1 p-10 overflow-auto">
        <div className="max-w-5xl mx-auto">
          <div className="flex justify-between items-center mb-10">
            ...
            {userRole === 'admin' && (
              <button onClick={handleExportCSV} className="bg-blue-600 text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2 hover:bg-blue-700 hover:-translate-y-1 transition-all shadow-xl shadow-blue-200">
            ...
            <div className="grid gap-8">
              <div className="bg-gradient-to-br from-green-500 to-green-700 text-white p-10 rounded-[2.5rem] shadow-2xl shadow-green-200 relative overflow-hidden">
                <div className="relative z-10">
                  ...
                  <button onClick={handleRecycle} className="bg-white text-green-700 px-10 py-5 rounded-2xl font-black hover:scale-105 transition-transform shadow-lg">
                    + DEPOSITAR BOTELLA
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  {recompensas.map(r => (
                    <div key={r.id} className="p-6 bg-gray-50 rounded-3xl border border-gray-100 flex flex-col justify-between hover:border-green-200 transition-colors">
                      ...
                      <button disabled={points < r.costo} onClick={() => handleRedeem(r.id, r.costo)} className={`mt-6 py-4 rounded-2xl font-black text-sm transition-all ${points >= r.costo ? 'bg-gray-900 text-white hover:bg-black shadow-lg shadow-gray-200' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>
            ...
            <div className="grid gap-8">
              ...
              <div className="flex items-center gap-4 bg-white p-5 rounded-3xl border border-gray-100 shadow-sm w-fit">
                ...
                <div>
                  ...
                  <p className="font-black text-gray-800 uppercase text-xs">SISTEMA: {systemHealth?.status || 'VERIFICANDO...'}</p>
                </div>
              </div>
                  ...
                  <tbody>
                    {ranking.map((u, i) => (
                      <tr key={i} className="hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-0">
                        <td className="p-5">
                          <span className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-xs ${i === 0 ? 'bg-yellow-100 text-yellow-600' : 'bg-gray-100 text-gray-500'}`}>{i + 1}</span>
                        </td>
                        <td className="p-5">
                          <div><p className="font-bold text-gray-800">{u.nombre}</p><p className="text-[10px] text-gray-400">{u.email}</p></div>
                        </td>
                        <td className="p-5 text-center">
                          <span className="bg-blue-50 text-blue-600 px-4 py-2 rounded-xl font-black text-sm">{u.puntos} pts</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
